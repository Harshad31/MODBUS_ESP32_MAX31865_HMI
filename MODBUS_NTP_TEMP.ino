...
void loop() {
  ...
  if (getLocalTime(&timeinfo)) {
    ...
    // Update Modbus holding registers with the current time values
    holdingRegisters[0] = timeinfo.tm_hour;  // Hours in Holding Register 1
    holdingRegisters[1] = timeinfo.tm_min;   // Minutes in Holding Register 2
    holdingRegisters[2] = timeinfo.tm_sec;   // Seconds in Holding Register 3
    holdingRegisters[3] = ms;                // Milliseconds in Holding Register 4
    ...
    holdingRegisters[4] = (uint16_t)(previousEMA * 10);  // Store the temperature in register
